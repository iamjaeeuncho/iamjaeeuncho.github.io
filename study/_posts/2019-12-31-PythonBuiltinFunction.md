...
noindex: true
---
<font color="white">［Python］파이썬 자주 쓰는 내장함수들</font>
...
# all: 요소 중 하나라도 거짓이면 False, 다 참이여야 True
...
# any: all(x)의 반대 경우 - 하나라도 참이면 True 리턴, 모두 거짓이면 False 리턴
```python
...
# chr: chr(i)는 아스키(ASCII) 코드값을 입력으로 받아 그 코드에 해당하는 문자를 출력하는 함수.
...
# enumerate: 순서값과 입력값을 enumerate 객체로 리턴
...
# id: id(object)는 객체를 입력받아 객체의 고유 주소값(레퍼런스)을 리턴하는 함수
```python
...
# input: input([prompt])은 사용자 입력을 받는 함수
...
# len: len(s)은 입력값 s의 길이(요소의 전체 개수)를 리턴하는 함수이다. 
```python
>>> len("python") 
6 
>>> len([1,2,3]) 
...
# list: list(s)는 반복 가능한 자료형 s를 입력받아 리스트로 만들어 리턴하는 함수이다. 
```python
>>> list("python") 
['p', 'y', 't', 'h', 'o', 'n'] 
>>> list((1,2,3)) 
...
# max: max(iterable)는 인수로 반복 가능한 자료형을 입력받아 그 최대값을 리턴하는 함수이다. 
```python
>>> max([1, 2, 3]) 
3 
>>> max("python") 
...
# min: min(iterable)은 max 함수와 반대로, 인수로 반복 가능한 자료형을 입력받아 그 최소값을 리턴하는 함수이다. 
```python
>>> min([1, 2, 3]) 
1 
>>> min("python") 
...
# oct: oct(x)는 정수 형태의 숫자를 8진수 문자열로 바꾸어 리턴하는 함수이다. 
```python
>>> oct(34) 
'0o42' 
>>> oct(12345) 
...
# ord: ord 함수는 chr 함수와 반대로, ord(c)는 문자의 아스키 코드값을 리턴하는 함수
```python
>>> ord('a') 
97 
>>> ord('0') 
...
# pow: pow(x, y)는 x의 y 제곱한 결과값을 리턴하는 함수
```python
>>> pow(2, 4) 
16 
>>> pow(3, 3) 
...
# str: str(object)은 문자열 형태로 객체를 변환하여 리턴하는 함수이다. 
```python
>>> str(3) 
'3' 
>>> str('hi') 
...
# sum: sum(iterable)은 입력으로 받은 리스트나 튜플의 모든 요소의 합을 리턴하는 함수이다. 
```python
>>> sum([1,2,3]) 
6 
>>> sum((4,5,6)) 
