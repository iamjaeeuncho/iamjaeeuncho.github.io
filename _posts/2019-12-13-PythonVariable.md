...
```python
>>> a = [1, 2, 3]
>>> id(a)                   # id함수는 변수가 가리키고 있는 객체의 주소를 리턴해 주는 파이썬 내장함수
4303029896                  # a변수가 가리키는 [1,2,3]이라는 리스트의 메모리 주소
```
- [1, 2, 3] 이라는 값을 가지는 리스트 자료형(객체)이 자동으로 메모리에 생성되고, 변수 a는 [1, 2, 3] 이라는 리스트가 저장된 메모리의 주소를 가리키게 된다.
...
a가 가리키는 리스트와는 다른 리스트를 가리키게 하는 2가지 방법
