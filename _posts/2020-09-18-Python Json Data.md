...
#### API의 역할?
1. API는 여러분이 가진 서버와 데이터베이스에 대한 출입구 역할을 하며, 허용된 사람들에게만 접근성을 부여해줍니다.
2. API는 애플리케이션과 기기가 데이터를 원활히 주고받을 수 있도록 돕는 역할을 합니다.
3. API는 모든 접속을 표준화하기 때문에 기계/ 운영체제 등과 상관없이 누구나 동일한 액세스를 얻을 수 있습니다. 쉽게 말해, API는 범용 플러그처럼 작동한다고 볼 수 있습니다.

#### API유형?
1) private API: 내부 API로, 제 3자에게 노출되지 않습니다.
2) public API: 개방형 API로, 모두에게 공개됩니다.
3) partner API: partner API는 기업이 데이터 공유에 동의하는 특정인들만 사용할 수 있습니다. 종종 파트너 회사 간에 소프트웨어를 통합하기 위해 사용됩니다.
...
print(endpoint)
# xml 형식으로 데이터를 받아옴.
# 요즘에는 보통 jason 형식으로 받아옴. 주소 마지막에 _type=json 추가
```
